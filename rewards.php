 ...
 * example file, the filename is "rewards" and therefore all functions
 * begin "addonmodule_".
 *
...
use WHMCS\Module\Addon\Rewards\Admin\AdminDispatcher;
use WHMCS\Module\Addon\Rewards\Client\ClientDispatcher;
 ...
 * @return array
 */
function rewards_config()
{
    return [
        // Display name for your module
        'name' => 'Rooster Grin Rewards Program',
        // Description displayed within the admin interface
        'description' => 'This module provides a method for Rooster Grin clients to examine their points and benefits.',
        // Module author name
        'author' => 'Cooper Timmerman',
        // Default language
        'language' => 'english',
        // Version number
        'version' => '1.0',
...
function rewards_activate()
{
    ...
    try {
        ...
        return [
            // Supported values here include: success, error or info
            'status' => "error",
            'description' => 'Unable to create mod_rewards: ' . $e->getMessage(),
        ];
    }
}
 ...
 * @return array Optional success/failure message
 */
function rewards_deactivate()
{
    // Undo any database and schema modifications made by your module here
    try {
        Capsule::schema()
            ->dropIfExists('mod_rewards');
        ...
        return [
            // Supported values here include: success, error or info
            "status" => "error",
            "description" => "Unable to drop mod_rewards: {$e->getMessage()}",
        ];
    }
}
...
function rewards_upgrade($vars)
{
    ...
    if ($currentlyInstalledVersion < 1.1) {
        ...
        $schema->table('mod_rewards', function($table) {
            $table->text('demo2');
        });
    }
 ...
 * @return string
 */
function rewards_output($vars)
{
    // Get common module parameters
    $modulelink = $vars['modulelink']; // eg. rewards.php?module=rewards
 ...
 * @return array
 */
function rewards_clientarea($vars)
{
    // Get common module parameters
    $modulelink = $vars['modulelink']; // eg. index.php?m=rewards
    $version = $vars['version']; // eg. 1.0
    $_lang = $vars['_lang']; // an array of the currently loaded language variables
